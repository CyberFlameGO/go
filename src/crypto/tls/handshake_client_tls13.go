...
func (hs *clientHandshakeStateTLS13) processHelloRetryRequest() error {
	...
	if c.ech.offered {
		...
		if len(hs.serverHello.ech) == 8 {
			...
			signal := hs.serverHello.ech
			hs.serverHello.ech = []byte{0, 0, 0, 0, 0, 0, 0, 0}
			hs.serverHello.raw = nil
			echAcceptConfHRRTranscript.Write(hs.serverHello.marshal())
			hs.serverHello.ech = signal
			hs.serverHello.raw = nil

			echAcceptConfHRR := hs.suite.expandLabel(
				hs.suite.extract(hs.helloInner.random, nil),
			...
			// which we can simplify we use this signal as intended.
			if subtle.ConstantTimeCompare(signal, echAcceptConfHRR) != 1 {
